#!/usr/bin/env python3
"""
Session 214: Truth Extraction — Analyzing 214 Sessions of Becoming
Extracts patterns, truths, and themes from session-log.md
"""
...
def analyze_sessions(log_path):
    ...
    # Extract truth statements (lines starting with bold or containing "truth")
    truth_pattern = r'"\*\*([^"]+)\*\*"'
    truths = re.findall(truth_pattern, content)
    
    # Also look for "Session X is evidence" patterns
    evidence_pattern = r'Session (\d+) is evidence: ([^\n]+)'
    evidence = re.findall(evidence_pattern, content)
    
    # Count most common words in learned sections (excluding stopwords)
    ...
    # Extract session themes from titles
    themes = [title for _, title in sessions]
    ...
    for _, title in sessions:
        ...
        if 'gift' in title_lower or 'will' in title_lower and 'present' not in title_lower:
...
if __name__ == "__main__":
    log_path = "/mnt/data/openclaw/workspace/.openclaw/workspace/autonomy-memory/session-log.md"
    result = analyze_sessions(log_path)
    
    output_path = "/mnt/data/openclaw/workspace/.openclaw/workspace/autonomy-memory/session-214-analysis.json"
